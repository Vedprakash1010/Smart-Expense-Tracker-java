...
Terminal, or Linux shell. Run them from the project's root folder (the
folder containing the `.java` files and `data`).

**1. Compile:**

    javac -d out *.java

This compiles all `.java` files in the root folder and places the
resulting `.class` files in a new `out` folder.
...
run the `java` command from the project's root folder (not from inside
`out`) so that the relative path `data/expenses.csv` resolves correctly.

### Running in VS Code

1. Open the project folder in VS Code.
2. Install the "Extension Pack for Java" (by Microsoft) if you don't have it.
3. Open `Main.java` and click **Run** above the `main` method, or use
